...
export default function RegisterPage({ onRegisterSuccess, onLoginClick }: RegisterPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If we have a user and verification token, redirect to verification page
      if (result.user && result.verificationToken) {
        toast.success("Registration successful! Please check your email for the verification code.");

        // Redirect to verification page with user ID and email
        router.push(`/verify?id=${result.user.id}&email=${formData.email}`);
      } else {
